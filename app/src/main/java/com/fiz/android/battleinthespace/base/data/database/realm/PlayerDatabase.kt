...
class PlayerDatabaseRealm(val databaseRealm: Realm) {
    ...
    fun get(id: Int): LiveData<Player?> {
        return MutableLiveData(databaseRealm.where<PlayerRealm>().equalTo("id", id).findFirst()?.asPlayer())
    }
    ...
    fun getAll(): LiveData<List<Player>?> {
        ...
        val alls = all.map { it.asPlayer() }
        return MutableLiveData<List<Player>>(alls)
    }
