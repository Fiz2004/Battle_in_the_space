...
class ListActors(
    ...
    private var respawns: MutableList<Respawn> = mutableListOf(
        Respawn(
            com.fiz.battleinthespace.common.Vec(width / 4.0, height / 4.0),
            angle = 45.0
        ),
        Respawn(
            com.fiz.battleinthespace.common.Vec(width - width / 4.0, height / 4.0),
            angle = 135.0
        ),
        Respawn(
            com.fiz.battleinthespace.common.Vec(width / 4.0, height - height / 4.0),
            angle = 315.0
        ),
        Respawn(
            com.fiz.battleinthespace.common.Vec(width - width / 4.0, height - height / 4.0),
            angle = 225.0
        )
    )
    ...
    fun createSpaceShips(countPlayers: Int) {
        if (spaceShips.isEmpty())
            ...
            spaceShips = spaceShips.mapIndexed { index, spaceShip ->
                spaceShip.copy(
                    center = com.fiz.battleinthespace.common.Vec(respawns[index].center),
